...
public class XmlCountryParser implements CountryParser {
	...
	/**
	 * Analisa países de um arquivo xml.
	 * 
	 * @param context
	 * @param rs
	 */
		...
		try {
			...
			while (true) {
			    ...
			    if (event == XMLStreamConstants.START_ELEMENT && parser.getLocalName().equals("country")) {
			    	...
			    	Country country = new Country(context, countryCode);
			    	country.setCountryName(parser.getAttributeValue("", "countryName").concat("                                ").substring(0, 32).trim());
